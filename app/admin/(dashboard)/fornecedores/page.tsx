...
export default async function SuppliersPage({ searchParams }: Props) {
  ...
  const suppliers = await db.supplier.findMany({ include: { _count: { select: { products: true, mappingTemplates: true } } }, orderBy: { name: "asc" } });
  return <div className="admin-page"><div className="admin-heading"><div><p className="eyebrow">Integrações</p><h1>Fornecedores</h1><p>Cadastre fontes para planilhas e associe adapters reais quando disponíveis.</p></div></div>
    ...
    <div className="mt-6 grid gap-4 xl:grid-cols-2">{suppliers.map((supplier) => <details className="admin-panel" key={supplier.id} open={raw.id === supplier.id}><summary className="cursor-pointer list-none"><div className="flex items-start justify-between gap-4"><div><h2>{supplier.name}</h2><p className="mt-1 text-sm text-muted">{supplier.adapterKey} · {supplier._count.products} produto(s) · {supplier._count.mappingTemplates} template(s)</p></div><span className={`status-pill ${supplier.active ? "active" : "inactive"}`}>{supplier.active ? "Ativo" : "Inativo"}</span></div></summary><SupplierForm supplier={supplier} /></details>)}</div>
  </div>;
}
