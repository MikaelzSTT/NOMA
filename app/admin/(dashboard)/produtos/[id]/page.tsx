    ...
    <div className="admin-page max-w-5xl">
      ...
      <form action={updateInternalProductAction} className="space-y-6">
        ...
        <section className="admin-panel space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><label className="admin-field">Estoque<input name="stock" type="number" min="0" step="1" defaultValue={selectedOffer?.stockQuantity ?? product.stock} /></label><label className="admin-field">Disponibilidade<select name="availability" defaultValue={selectedOffer?.availability ?? product.availability}><option value="AVAILABLE">Disponível</option><option value="OUT_OF_STOCK">Sem estoque</option><option value="PREORDER">Pré-venda</option><option value="UNKNOWN">Não informada</option></select></label><MoneyField name="shippingCost" label="Custo de frete" value={selectedOffer?.shippingCost == null ? null : Number(selectedOffer.shippingCost)} /><label className="admin-field">Prazo estimado<input name="estimatedDelivery" defaultValue={selectedOffer?.estimatedDelivery ?? product.estimatedDelivery ?? ""} /></label></div>
