...
export function Breadcrumbs({ items, market = "BR" }: { items: Array<{ label: string; href?: string }>; market?: Market }) {
  return (
    <nav aria-label="Navegação estrutural" className="product-breadcrumbs flex flex-wrap items-center gap-1 py-5 text-xs text-muted">
      <Link href={marketHomePath(market)} className="hover:text-brand">{market === "US" ? "Home" : "Início"}</Link>
      {items.map((item) => (
